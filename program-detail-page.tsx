              ...
              <div className="grid grid-cols-2 gap-4 mt-6">
                {program.highlights.map((highlight, index) => {
                  ...
                  return (
                    <div
                      key={highlight.label}
                      className={`flex items-center space-x-3 p-3 rounded-lg ${isEven ? "bg-turquoise-50" : "bg-green-50"}`}
                    >
                      {isEven ? (
                      ...
                      <div>
                        <span
                          className={`text-xs block font-medium ${isEven ? "text-turquoise-600" : "text-green-600"}`}
                        >
                          {highlight.label}
                        </span>
                        <span
                          className={`text-sm font-bold ${isEven ? "text-turquoise-900" : "text-green-900"}`}
                        >
                          {highlight.value}
                        </span>
                      </div>
          ...
          <CardContent>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              {program.offerings.map((offering) => {
                ...
                return (
                  <div key={offering.title} className={`text-center p-6 rounded-xl ${offering.bgColor} border border-gray-200`}>
          ...
          <CardHeader>
            <CardTitle className="text-xl text-gray-900 flex items-center justify-between">
              ...
              <span className="text-sm font-normal text-gray-600 bg-gray-100 px-3 py-1 rounded-full">
                {activeOpportunities.length} disponibles
              </span>
            </CardTitle>
            ...
            {activeOpportunities.length > 0 ? (
                      ...
                      <div className="flex flex-col sm:flex-row sm:items-center sm:justify-between gap-4">
                        <div className="flex-1">
                          <div className="flex items-center gap-2 mb-2">
                            <h3 className="font-bold text-gray-900">{opportunity.detailTitle}</h3>
                            <span
                          ...
                          <p className="text-gray-600 text-sm mb-2">{opportunity.summary}</p>
              ...
              <div className="text-center py-8">
                <p className="text-gray-600">
                  Aún no hay oportunidades publicadas para este programa. Suscríbete para recibir notificaciones cuando
                  abra la próxima convocatoria.
                </p>
              </div>
            )}
